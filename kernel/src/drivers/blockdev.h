 ...
 * fixed-size sectors from/to by LBA" -- NVMe today, virtio-blk or
 * anything else tomorrow, all behind the exact same four calls.
 * fs/graph.c's persistence layer (graph_save_to_disk()/
 * graph_load_from_disk()) goes through this instead of calling
 * drivers/nvme.h directly, so it never needs to change when a second
 * driver shows up -- that driver just registers itself here.
 *
