...
static void dump_and_panic(struct interrupt_frame *f) {
  ...
  kprintf("\n*** UNHANDLED EXCEPTION %u: %s (error=0x%x) ***\n", (unsigned)f->vector, exception_name(f->vector), (unsigned)f->error_code);
  if (f->vector == 14) {
...
/* Tiny exception table; a linear scan is sufficient. */
...
/*
 * Redirect known fault sites to their recovery code.
 * iretq then resumes at the fixup instead of the faulting instruction.
 */
...
void isr_common_handler(struct interrupt_frame *frame) {
  ...
  // Already acknowledged; report the unhandled vector.
